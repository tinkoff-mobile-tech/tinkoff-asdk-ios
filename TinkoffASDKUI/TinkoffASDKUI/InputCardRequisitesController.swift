...
class InputCardRequisitesController: NSObject {
    ...
    private func activateScanerButton() {
        if cardRequisitesScaner != nil {
            ...
            inputView?.buttonRight.setImage(UIImage(named: "scan", in: Bundle(for: type(of: self)), compatibleWith: nil), for: .normal)
    ...
    private func activateNextButton() {
        inputView?.buttonRight.isHidden = false
        inputView?.buttonRight.setImage(UIImage(named: "next", in: Bundle(for: type(of: self)), compatibleWith: nil), for: .normal)
