...
open class MaskedTextFieldDelegate: NSObject, UITextFieldDelegate {
	...
	@IBInspectable public var maskFormat: String {
		...
		set(newFormat) {
			...
			self.mask = try! InputMask.getOrCreate(withFormat: newFormat)
		}
	}
	...
	open func put(text: String, into field: UITextField) {
		let result: InputMask.Result = self.mask.apply(
	...
	open func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
		...
		self.listener?.textField?(textField, didFillMask: complete, extractValue: extractedValue)
		_ = self.listener?.textField?(textField, shouldChangeCharactersIn: range, replacementString: string)
	...
	open func textFieldShouldClear(_ textField: UITextField) -> Bool {
		...
		if shouldClear {
			...
			self.listener?.textField?(textField, didFillMask: result.complete, extractValue: result.extractedValue)
		}
