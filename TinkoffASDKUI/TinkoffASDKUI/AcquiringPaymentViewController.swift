...
class AcquiringPaymentViewController: PopUpViewContoller {
	...
	override func pushToNavigationStackAndActivate(firstResponder textField: UIView?, completion: (() -> Void)? = nil) -> Bool {
	...
	private func registerCells(_ names: [String], for table: UITableView) {
		names.forEach { (name) in
			table.register(UINib.init(nibName: name, bundle: Bundle(for: type(of: self))), forCellReuseIdentifier: name)
		}
	}
	...
	private func updateTableViewCells() {
		...
		switch paymentStatus {
			...
			case .qrCodeStatic(let qrCode, let title):
			...
			case .paymentWaitingSBPQrCode(let qrCode, let status):
				tableViewCells.append(.waitingPaymentQrCode(qrCode: qrCode, status: status))
			
			case .paymentWaitingSBPUrl(let url, let status):
		...
		switch tableViewCells[indexPath.row] {
			...
			case .cardList:
				if let cell = tableView.dequeueReusableCell(withIdentifier: "ScrollableTableViewCell") as? ScrollableTableViewCell {
					...
					if case .paymentWainingCVC(let parentPaymentId) = paymentStatus {
			...
			case .error:
				if let cell = tableView.dequeueReusableCell(withIdentifier: "StatusTableViewCell") as? StatusTableViewCell {
					...
					if case .error(let error) = paymentStatus {
			...
			case .waitingPaymentQrCode(let qrCode, let status):
			...
			case .buttonPay:
				if let cell = tableView.dequeueReusableCell(withIdentifier: "ButtonTableViewCell") as? ButtonTableViewCell {
					...
					cell.buttonAction.tintColor = UIColor.init(hex: "#333333")
					cell.buttonAction.backgroundColor = UIColor.init(hex: "#FFDD2D")
			...
			case .buttonPaySBP:
				if let cell = tableView.dequeueReusableCell(withIdentifier: "ButtonTableViewCell") as? ButtonTableViewCell {
					...
					cell.setButtonIcon(UIImage.init(named: "buttonIconSBP", in: Bundle.init(for: type(of: self)), compatibleWith: nil))
...
extension AcquiringPaymentViewController: AcquiringView {
	...
	func cardRequisites() -> PaymentSourceData? {
		switch cardListPresenter.requisies() {
			case .savedCard(let card, let cvv):
