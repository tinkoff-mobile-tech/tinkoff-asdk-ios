...
final class ThreeDSViewController<Payload: Decodable>: UIViewController, WKNavigationDelegate {
    ...
    func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
        webView.evaluateJavaScript("document.baseURI") { value, error in
            ...
            webView.evaluateJavaScript("document.getElementsByTagName('pre')[0].innerText") { [weak self] value, error in
    ...
    private func setupCloseButton() {
        ...
        if #available(iOS 13.0, *) {
            cancelButton = UIBarButtonItem(barButtonSystemItem: .close,
                                           target: self,
                                           action: #selector(didTapCloseButton))
        } else {
            cancelButton = UIBarButtonItem(title: AcqLoc.instance.localize("TinkoffAcquiring.button.close"),
                                           style: .done,
                                           target: self,
                                           action: #selector(didTapCloseButton))
        }
