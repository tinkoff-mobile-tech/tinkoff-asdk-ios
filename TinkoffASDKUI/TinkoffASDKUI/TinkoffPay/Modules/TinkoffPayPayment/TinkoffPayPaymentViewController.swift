...
private extension TinkoffPayPaymentViewController {
    ...
    func performTinkoffPayWith(
        paymentId: Int64,
        version: GetTinkoffPayStatusResponse.Status.Version
    ) {
        _ = tinkoffPayController.getTinkoffPayLink(
            paymentId: paymentId,
            version: version,
    ...
    func handleTinkoffAppNotInstalled() {
        ...
        let alertController = UIAlertController(
            title: Loc.Tp.NoTinkoffBankApp.title,
            message: Loc.Tp.NoTinkoffBankApp.description,
            preferredStyle: .alert
        )
        let installAction = UIAlertAction(
            title: Loc.Tp.NoTinkoffBankApp.Button.install,
            style: .default
        ) { [weak self] _ in
        ...
        let cancelAction = UIAlertAction(
            title: Loc.Tp.NoTinkoffBankApp.Button.сancel,
            style: .cancel
        ) { [weak self] _ in
            self?.dismiss(animated: true)
        }
