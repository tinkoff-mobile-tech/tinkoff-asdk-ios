...
public enum PaymentSourceData: Codable {
	...
	public func encode(to encoder: Encoder) throws {
		...
		switch self {
		case .cardNumber(let number, let expData, let cvv):
		...
		case .savedCard(let cardId, let cvv):
			try container.encode(cardId, forKey: .savedCardId)
			try container.encode(cvv, forKey: .cardCVV)
			
		case .paymentData(let data):
...
public struct DeviceInfoParams: Codable {
	...
	enum CodingKeys: String, CodingKey {
		case threeDSCompInd = "threeDSCompInd"
		...
		case cresCallbackUrl = "cresCallbackUrl"
	}
...
public class PaymentFinishRequest: RequestOperation, AcquiringRequestTokenParams {
	...
	public init(data: PaymentFinishRequestData) {
		self.parameters = [:]
		self.parameters?.updateValue(data.paymentId, forKey: PaymentFinishRequestData.CodingKeys.paymentId.rawValue)
		
		if let value = data.sendEmail {
			self.parameters?.updateValue(value, forKey: PaymentFinishRequestData.CodingKeys.sendEmail.rawValue)
		}
		
		if let value = data.infoEmail {
			self.parameters?.updateValue(value, forKey: PaymentFinishRequestData.CodingKeys.infoEmail.rawValue)
		}
		
		if let ip = data.ipAddress {
			self.parameters?.updateValue(ip, forKey: PaymentFinishRequestData.CodingKeys.ipAddress.rawValue)
		}
		
		if let deviceInfo = data.deviceInfo, let value = try? deviceInfo.encode2JSONObject() {
			self.parameters?.updateValue(value, forKey: PaymentFinishRequestData.CodingKeys.deviceInfo.rawValue)
		}
		
		if let source = data.source {
			self.parameters?.updateValue(source, forKey: PaymentFinishRequestData.CodingKeys.source.rawValue)
		}
		
		if let route = data.route {
			self.parameters?.updateValue(route, forKey: PaymentFinishRequestData.CodingKeys.route.rawValue)
		}
		
		switch data.paymentSource {
		case .cardNumber(let number, let expDate, let cvv):
			...
			self.parameters?.updateValue(value, forKey: PaymentFinishRequestData.CodingKeys.cardData.rawValue)
		...
		case .paymentData(let token):
			self.parameters?.updateValue(token, forKey: PaymentFinishRequestData.CodingKeys.encryptedPaymentData.rawValue)
...
public struct PaymentFinishResponse: ResponseOperation {
	...
	public init(from decoder: Decoder) throws {
		...
		case .authorized, .confirmed, .checked3ds:
			if let finishStatus = try? PaymentStatusResponse.init(from: decoder) {
