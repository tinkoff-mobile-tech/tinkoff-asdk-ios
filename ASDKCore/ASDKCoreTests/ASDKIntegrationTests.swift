...
import XCTest
@testable import ASDKCore

class ASDKIntegrationTests: XCTestCase {
	...
	override func setUp() {
		let credential = AcquiringSdkCredential.init(terminalKey: ASDKStageTestData.terminalKey,
	...
	*/
	func test_FinishRequest() {
		...
		let paymentData = PaymentFinishRequestData.init(paymentId: 1, paymentSource: PaymentSourceData.cardNumber(number: "5182230000000010", expDate: "1122", cvv: "111"))
	...
	func test_Pay() {
		...
		_ = sdk?.paymentInit(data: paymentData, completionHandler: { (response) in
			...
			case .success(let response):
				...
				/// Данные платежной карты сформированы на основе тестовых данных с `https://oplata.tinkoff.ru/landing/develop/test`
				let paymentData = PaymentFinishRequestData.init(paymentId: response.paymentId, paymentSource: PaymentSourceData.cardNumber(number: "5182230000000010", expDate: "1122", cvv: "111"))
