...
class InputCardRequisitesController: NSObject {
	...
	private func activateScanerButton() {
		if cardRequisitesScaner != nil {
			...
			inputView?.buttonRight.setImage(UIImage.init(named: "scan", in: Bundle(for: type(of: self)), compatibleWith: nil), for: .normal)
	...
	private func activateNextButton() {
		inputView?.buttonRight.isHidden = false
		inputView?.buttonRight.setImage(UIImage.init(named: "next", in: Bundle(for: type(of: self)), compatibleWith: nil), for: .normal)
...
extension InputCardRequisitesController: MaskedTextFieldDelegateListener {
	...
	func textField(_ textField: UITextField, didFillMask complete: Bool, extractValue value: String) {
		if inputView?.textFieldCardNumber == textField {
			...
			cardRequisitesBrandInfo.cardBrandInfo(numbers: value, completion: { [weak self] (cardNumber, icon, size) in
...
extension InputCardRequisitesController: InputCardRequisitesDataSource {
	...
	{
		self.becomeFirstResponderListener = responderListener
		self.inputView = inputView
		self.inputAccessoryViewWithButton = inputAccessoryView
		self.inputAccessoryViewWithButton?.onButtonTouchUpInside = { [weak self] in
			self?.onButtonDoneTouchUpInside()
		}
		
		self.cardRequisitesScaner = scaner
